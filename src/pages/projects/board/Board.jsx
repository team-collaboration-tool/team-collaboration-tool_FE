...
export default function Board() {
  ...
  React.useEffect(() => {
    ...
    postsArray.sort((a, b) => {
      const idA = parseInt(
        a.getElementsByTagName("id")[0].textContent || "0",
        10
      );
      const idB = parseInt(
        b.getElementsByTagName("id")[0].textContent || "0",
        10
      );
      return idB - idA;
    });
    ...
    const buildPostItem = (post, isNotice) => {
      ...
      const timestamp =
        post.getElementsByTagName("timestamp")[0].textContent || "";
      ...
      voteSpan.className = `post_ishaveVote ${
        hasVote ? "has-vote" : "no-vote"
      }`;
    ...
    const getPostDataById = (idStr) => {
      const el = postsArray.find(
        (p) =>
          (p.getElementsByTagName("id")[0]?.textContent || "").trim() === idStr
      );
      if (!el) return null;
      const t = (tag) =>
    ...
    const renderSee = (post) => {
      ...
      completeBtn &&
        completeBtn.addEventListener("click", () => {
          ...
          checks.forEach((checkbox, index) => {
            if (checkbox.checked) {
              if (VoteResult_NumberResult[index] == null)
                VoteResult_NumberResult[index] = 0;
              VoteResult_NumberResult[index]++;
              if (!VoteResult_Option_who[index])
                VoteResult_Option_who[index] = [];
      ...
      const numBtns = seeContainer.querySelectorAll(
        ".VoteResult_item_PeopleNumber"
      );
      numBtns.forEach((btn, idx) => {
        btn.addEventListener("click", () => {
          const whoPanels = seeContainer.querySelectorAll(
            ".VoteResult_item_WhoVoted"
          );
    ...
    const onSubmitWrite = () => {
      ...
      const value_PostMake_files =
        files && files.length > 0 ? fileNames : "없음";
    ...
    const onMakeVote = () => {
      ...
      const reqOptions =
        optionContainer?.querySelectorAll("input[required]") || [];
      ...
      const value_VoteMake_isitMulti =
        document.getElementById("VoteMake_isitMulti")?.checked || false;
      const value_VoteMake_isitAnonymous =
        document.getElementById("VoteMake_isitAnonymous")?.checked || false;
      ...
      <div className="GaeSiPan_Write">
        ...
        <div className="GSPW_field">
          <div className="checkbox_container">
            ...
            <label htmlFor="is-notice" className="checkbox_label">
              공지사항 유무
            </label>
          </div>
        </div>
        ...
        <div className="GSPW_field">
          <label htmlFor="file-upload" className="form_label">
            첨부파일
          </label>
          <input
            type="file"
        ...
        <div className="VOTE_Make">
          ...
          <div className="VoteMake_item">
            ...
            <button type="button" id="VoteMake_AddOption">
              선택지 추가
            </button>
          </div>
          ...
          <button type="submit" id="VoteMake_button">
            투표 생성
          </button>
        </div>
      </div>
